...
- В конфигурации воркера/endpoint выберите GPU с `CUDA SM 8.9+`, рекомендуемо `H100/H200/L40S`
...
- Воркер зафиксирован под FP8-путь (`CUDA SM 8.9+`):
  - на GPU ниже `SM 8.9` он завершится с явной ошибкой в startup/runtime
  - логика инференса фиксированная: `FP8 + AOTI` без runtime fallback
